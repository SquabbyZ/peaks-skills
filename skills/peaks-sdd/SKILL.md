...
```
Step 1: AskUserQuestion 确认项目名称 + 创建目录
   ↓ 用户确认项目名（如 "AI Chat"）
   ↓ 立即创建 {{PROJECT_NAME}}/ 目录
Step 2: 调用 PeaksSDD-product agent 进行脑暴
   ↓ 产出 {{PROJECT_NAME}}/.peaks/prds/brainstorm-[日期].md（必须）
   ↓ 脑暴未完成不进入下一步
Step 3: AskUserQuestion 确认技术栈
   ↓ 用户选择或自定义（前后端项目默认 Monorepo）
Step 4: 调用 CLI 创建项目 + 初始化 .claude/.peaks/
```
...
后续所有产出物都放在项目目录下，避免散落。

#### Step 2: 调用 PeaksSDD-product agent 脑暴
...
使用 `init.mjs` 脚本完成初始化，传入技术栈参数：

```bash
# 获取 SKILL_PATH（peaks-sdd skill 目录）
SKILL_PATH=~/.claude/skills/peaks-sdd  # 或本地开发路径

# 单包项目初始化
node $SKILL_PATH/scripts/init.mjs . --frontend=react --ui=shadcn --backend=nestjs --database=postgresql

# Monorepo 项目初始化（前后端项目默认）
node $SKILL_PATH/scripts/init.mjs . --frontend=react --ui=shadcn --backend=nestjs --database=postgresql --monorepo=true
```

`init.mjs` 会自动：
1. 创建 .peaks 目录结构（在项目根目录）
2. 创建 .claude/agents/ 目录
3. 扫描技术栈并生成完整的 Agent 配置
4. 配置 MCP 服务器（settings.json）
5. 创建 session-state.json

**Monorepo 目录结构**：

```
{{PROJECT_NAME}}/                    # 项目根目录（已 cd 进入）
├── packages/
...
│   └── agents/
├── pnpm-workspace.yaml
└── package.json                   # root package.json（workspace 配置）
```
