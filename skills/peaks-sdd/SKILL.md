...
Step 4: AskUserQuestion 确认技术栈
   ↓ 用户选择或自定义（根据 PRD 建议）
Step 5: 调用 CLI 创建项目（使用官方方式）
   ↓ 根据 PRD 确定的技术方案
   ↓ 使用官方 CLI 工具创建项目
Step 6: 初始化 .claude/.peaks/ 等配置
   ↓ 调用 init.mjs 完成项目初始化
```
...
`init.mjs` 会自动：
1. 扫描技术栈并生成完整的 Agent 配置（frontend、backend、qa 等）
2. 配置 MCP 服务器（settings.json）
3. 创建 session-state.json
4. 创建 .peaks/ 目录结构（如不存在）

**注意**：PRD 文件（.peaks/prds/prd-*.md）在 Step 3 已生成，init.mjs 不会覆盖。
...
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ Step 4: AskUserQuestion 确认技术栈（基于 PRD 建议）               │
