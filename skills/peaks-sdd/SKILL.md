---
name: peaks-sdd
description: Spec-Driven Development workflow. Use when running /peaksinit, /peaksfeat, /peaksbug, or when user mentions 初始化项目, setup project, constitution, PRD, technical plan, task breakdown.
user-invocable: true
---
...
|---------|------|---------|
| `/peaksinit` | `peaksinit.md` | Project initialization, tech stack detection, dynamic agent generation |
| `/peaksfeat` | `peaksfeat.md` | Feature development workflow (Constitution → PRD → Design → Develop → Review → QA → Deploy) |
| `/peaksbug` | `peaksbug.md` | Bug fixing workflow (Reproduce → Root cause → Fix → Regression test → Report) |
| `/peakscheck` | `peakscheck.md` | Check for peaks-sdd updates |
| `/peaksupdate` | `peaksupdate.md` | Update peaks-sdd to latest version |

**Usage:** When user runs `/peaksbug`, `/peaksinit`, or `/peaksfeat`, follow the corresponding command file.
...
```
SKILL.md (本文件：总览、决策树、Phase 概述)
    ↓ 用户调用 /peaksinit, /peaksfeat, /peaksbug
peaksinit.md, peaksfeat.md, peaksbug.md 等 (轻量包装器，解析参数后调用 Agent)
    ↓ Agent tool 调度
templates/agents/*.md (完整工作流定义，专家执行细节)
    ↓ 产出物
.peaks/ (PRD, Plan, Swagger, 测试, 报告等)
```

**设计原则**：SKILL.md 只描述"是什么"和"怎么选"，peaksinit.md/peaksfeat.md/peaksbug.md 负责"怎么触发"，templates/agents/ 负责"怎么执行"。避免三层重复描述同一内容。
...
| `peaksinit.md` | /peaksinit 命令实现 |
| `peaksfeat.md` | /peaksfeat 命令实现（轻量包装器，调用 Agent） |
| `peaksbug.md` | /peaksbug 命令实现（轻量包装器，调用 Agent） |
